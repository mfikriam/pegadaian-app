...
},{"axios":"../../node_modules/axios/index.js","./alert":"alert.js"}],"classification.js":[function(require,module,exports) {
"use strict";
...
//***************** Local Functions ********************/
var _countAttr = function _countAttr(dataset, attr, value) {
  return dataset.reduce(function (acc, pegawai) {
    return pegawai[attr] === value ? acc + 1 : acc;
  }, 0);
};
var _calcAttrProbability = function _calcAttrProbability(dataset) {
  var jabatanOptionValues = ['Generalis', 'Officer Kinerja', 'TL Teknik', 'TL Rensis', 'Team Leader Administrasi dan Umum', 'TL SAR DAN YAN GAN', 'JTC  Pengaturan Operasi', 'Of Kinerja Transaksi Energi Listrik', 'Plt. Manager ULP', 'JO Pengendalian Piutang'];
  var masaKerjaOptionValues = ['< 1 Tahun', '1 - 3 Tahun', '4 - 6 Tahun', '7 - 10 Tahun', '> 10 Tahun'];
  var gradeGajiOptionValues = ['Grade 7 - 8', 'Grade 9 - 11', 'Grade 12 - 13'];
  var statusNikahOptionValues = ['Menikah', 'Belum Menikah', 'Cerai'];
  var jumlahAnakOptionValues = ['0 Anak', '1 - 2 Anak', '3 Anak', '> 3 Anak'];
  var hutangTempatLainOptionValues = ['Ya', 'Tidak'];
  var kelayakanOptionValues = ['Layak', 'Tidak Layak'];
  var probObj = {
    jabatan: jabatanOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'jabatan', key) / _countAttr(dataset, 'jabatan', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'jabatan', key) / _countAttr(dataset, 'jabatan', key);
      return obj;
    }, {}),
    masa_kerja: masaKerjaOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'masa_kerja', key) / _countAttr(dataset, 'masa_kerja', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'masa_kerja', key) / _countAttr(dataset, 'masa_kerja', key);
      return obj;
    }, {}),
    grade_gaji: gradeGajiOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'grade_gaji', key) / _countAttr(dataset, 'grade_gaji', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'grade_gaji', key) / _countAttr(dataset, 'grade_gaji', key);
      return obj;
    }, {}),
    status_nikah: statusNikahOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'status_nikah', key) / _countAttr(dataset, 'status_nikah', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'status_nikah', key) / _countAttr(dataset, 'status_nikah', key);
      return obj;
    }, {}),
    jumlah_anak: jumlahAnakOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'jumlah_anak', key) / _countAttr(dataset, 'jumlah_anak', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'jumlah_anak', key) / _countAttr(dataset, 'jumlah_anak', key);
      return obj;
    }, {}),
    hutang_tempat_lain: hutangTempatLainOptionValues.reduce(function (obj, key) {
      obj[key] = {};
      obj[key]['Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Layak';
      }), 'hutang_tempat_lain', key) / _countAttr(dataset, 'hutang_tempat_lain', key);
      obj[key]['Tidak Layak'] = _countAttr(dataset.filter(function (el) {
        return el.kelayakan === 'Tidak Layak';
      }), 'hutang_tempat_lain', key) / _countAttr(dataset, 'hutang_tempat_lain', key);
      return obj;
    }, {}),
    kelayakan: kelayakanOptionValues.reduce(function (obj, key) {
...
var _naiveBayesPrediction = function _naiveBayesPrediction(probObj, pengaju) {
  var layakValue = probObj.kelayakan['Layak'] * probObj.jabatan[pengaju.jabatan]['Layak'] * probObj.masa_kerja[pengaju.masa_kerja]['Layak'] * probObj.grade_gaji[pengaju.grade_gaji]['Layak'] * probObj.status_nikah[pengaju.status_nikah]['Layak'] * probObj.jumlah_anak[pengaju.jumlah_anak]['Layak'] * probObj.hutang_tempat_lain[pengaju.hutang_tempat_lain]['Layak'];
  var tidakLayakValue = probObj.kelayakan['Tidak Layak'] * probObj.jabatan[pengaju.jabatan]['Tidak Layak'] * probObj.masa_kerja[pengaju.masa_kerja]['Tidak Layak'] * probObj.grade_gaji[pengaju.grade_gaji]['Tidak Layak'] * probObj.status_nikah[pengaju.status_nikah]['Tidak Layak'] * probObj.jumlah_anak[pengaju.jumlah_anak]['Tidak Layak'] * probObj.hutang_tempat_lain[pengaju.hutang_tempat_lain]['Tidak Layak'];
  return layakValue > tidakLayakValue ? 'Layak' : 'Tidak Layak';
};
var _calculateAccuracy = function _calculateAccuracy(probObj, dataTest) {
  dataTest.forEach(function (pegawai) {
    pegawai['prediksi_kelayakan'] = _naiveBayesPrediction(probObj, pegawai);
  });
  var accuracy = dataTest.reduce(function (acc, pegawai) {
    return pegawai.kelayakan === pegawai.prediksi_kelayakan ? acc += 1 : acc;
  }, 0) / dataTest.length;
  return accuracy;
};

//***************** Exported Functions ********************/
var naiveBayes = exports.naiveBayes = function naiveBayes(dataTrain, dataTest, pengaju) {
  var probObj = _calcAttrProbability(dataTrain);
  var prediksi_kelayakan = _naiveBayesPrediction(probObj, pengaju);
  var akurasi = _calculateAccuracy(probObj, _toConsumableArray(dataTest));
  return {
    prediksi_kelayakan: prediksi_kelayakan,
    akurasi: akurasi
  };
};
...
var pengajuTable = document.querySelector('#pengaju-table');
var addPengajuForm = document.querySelector('#form-add-pengaju');
var updatePengajuBtns = document.querySelectorAll('.btn-update-pengaju');
var deletePengajuBtns = document.querySelectorAll('.btn-delete-pengaju');
...
if (pengajuTable) {
  ...
  new _simpleDatatables.DataTable(pengajuTable, _options);

  //? Get Data Pegawai
  var pegawaiObjArr = JSON.parse(document.querySelector('#title-pengaju').dataset.pegawaiObjArr);
...
//? Add Data Pengaju
if (addPengajuForm) {
  var addPengajuModal = document.querySelector('#modal-add-pengaju');
  var bsAddPengajuModal = new bootstrap.Modal(addPengajuModal);
  addPengajuForm.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    if (addPengajuForm.checkValidity()) {
      ...
      //? Naive Bayes
      var _naiveBayes = (0, _classification.naiveBayes)(dataTrain, dataTest, pengajuObj),
        prediksi_kelayakan = _naiveBayes.prediksi_kelayakan,
        akurasi = _naiveBayes.akurasi;
      pengajuObj['prediksi_kelayakan'] = prediksi_kelayakan;
      pengajuObj['akurasi'] = akurasi;
      (0, _manageData.addNewData)('pengaju', pengajuObj, addPengajuForm, bsAddPengajuModal);
    }
  });
}
...
if (updatePengajuBtns.length > 0) {
  var updatePengajuModalList = document.querySelectorAll('[id^="modal-update-pengaju"]');
  var bsUpdatePengajuModalList = Array.from(updatePengajuModalList).map(function (el) {
    return new bootstrap.Modal(el);
  });
  var _updateDataFormList = document.querySelectorAll("[id^=\"form-update-pengaju\"]");
  _updateDataFormList.forEach(function (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      form.classList.add('was-validated');
      var pengajuId = form.dataset.objId;
      if (form.checkValidity()) {
        var pengajuObj = {
          nama: form.querySelector('#update-nama').value,
          jabatan: form.querySelector('#update-jabatan').value,
        ...
        //? Naive Bayes
        var _naiveBayes2 = (0, _classification.naiveBayes)(dataTrain, dataTest, pengajuObj),
          prediksi_kelayakan = _naiveBayes2.prediksi_kelayakan,
          akurasi = _naiveBayes2.akurasi;
        pengajuObj['prediksi_kelayakan'] = prediksi_kelayakan;
        pengajuObj['akurasi'] = akurasi;
        (0, _manageData.updateDataById)('pengaju', pengajuId, pengajuObj, form, bsUpdatePengajuModalList);
      }
    });
  });
}
...
if (deletePengajuBtns.length > 0) {
  var deletePengajuModalList = document.querySelectorAll('[id^="modal-delete-pengaju"]');
  var bsDeletePengajuModalList = Array.from(deletePengajuModalList).map(function (el) {
    return new bootstrap.Modal(el);
  });
  deletePengajuBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      var pengajuId = btn.dataset.objId;
      (0, _manageData.delDataById)('pengaju', pengajuId, bsDeletePengajuModalList);
    });
  });
}
